...
function CreatePostPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-16">
      ...
      <form onSubmit={handleSubmit} className="card-elevated mt-6 space-y-6 rounded-2xl border border-line bg-panel p-6">
        {/* Image */}
        <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <label className="block">
            ...
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 w-full rounded-lg border border-line bg-panel px-3 py-2 text-sm text-ink outline-none focus:border-signal">
          ...
          <label className="block">
            ...
            <select value={city} onChange={(e) => setCity(e.target.value)} className="mt-1 w-full rounded-lg border border-line bg-panel px-3 py-2 text-sm text-ink outline-none focus:border-signal">
              {CITY_OPTIONS.map((c) => (
                <option key={c} value={c}>{c}</option>
              ))}
            </select>
        ...
        <button
          ...
          className="brand-gradient flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60"
        >
          {isSavingPost ? <Loader size={16} className="animate-spin" /> : null}
